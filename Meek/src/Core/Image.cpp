...
Meek::Image::Image(const std::string& path, bool flip)
{
	int w, h, bits;
...
Meek::Image::Image(uint32_t width, uint32_t height, const void* data)
{
	uint32_t tex_id;
	glGenTextures(1, &tex_id);
	glBindTexture(GL_TEXTURE_2D, tex_id);
...
void Meek::Image::Invalidate(const void* data)
{
...
ImTextureID Meek::Image::GetID()
{
	return m_ID;
}

float Meek::Image::GetWidth()
{
	return m_Width;
}

float Meek::Image::GetHeight()
{
	return m_Height;
}
